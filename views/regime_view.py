...
class RegimeView(ttk.Frame):
    def __init__(self, parent, app, title: str, regime: str) -> None:
        ...
        filters=ttk.Frame(self);filters.pack(fill="x",pady=(0,8));self.search=__import__('tkinter').StringVar();self.year=__import__('tkinter').StringVar(value=str(date.today().year))
        ...
        columns=("cuit","actividad","estado","alta","responsable","ultimo_control","vencimiento","riesgo","ventas_anio","compras_anio","ventas_12","compras_12","iva_debito","iva_credito","pendientes")
        ...
        labels=("CUIT/CUIL","Actividad","Estado","Fecha alta","Responsable","Último control","Próximo vencimiento","Riesgo","Ventas año","Compras año","Ventas 12 meses","Compras 12 meses","IVA débito est.","IVA crédito est.","Pendientes")
        widths=(105,170,80,90,100,100,115,80,115,115,120,120,115,115,90)
    ...
    def refresh(self) -> None:
        ...
        term=self.search.get().casefold().strip();clients=[row for row in self._clients() if not term or term in f"{row['nombre_razon_social']} {row['cuit_cuil']}".casefold()]
        self.rows=[]
        for client in clients:
            ...
            metrics=self.app.database.query_one(
                ...
                (client_id,f"{year:04d}-%",client_id,f"{year:04d}-%",client_id,client_id,client_id),
            )
            ...
            row={**client,"ventas_anio":sales,"compras_anio":purchases,"ventas_12":float(metrics["ventas_12"] or 0),"compras_12":float(metrics["compras_12"] or 0),"iva_debito":sales*.21,"iva_credito":purchases*.21,"pendientes":int(metrics["pendientes"] or 0),"ultimo_control":ledger.get("ultimo_control",""),"fecha_alta":ledger.get("fecha_alta_estudio","")};self.rows.append(row)
        for item in self.tree.get_children():self.tree.delete(item)
        for row in self.rows:
            self.tree.insert("","end",iid=str(row["cliente_id"]),text=row["nombre_razon_social"],values=(row["cuit_cuil"],row.get("actividad",""),row.get("estado",""),display_date(row.get("fecha_alta")),row.get("responsable_interno",""),display_date(row.get("ultimo_control")),display_date(row.get("proximo_vencimiento")),row.get("riesgo_general",""),money(row["ventas_anio"]),money(row["compras_anio"]),money(row["ventas_12"]),money(row["compras_12"]),money(row["iva_debito"]),money(row["iva_credito"]),row["pendientes"]))
